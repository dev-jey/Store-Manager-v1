...
class Sales_Model():
    ...
    def __init__(self, userId=None, product=None, quantity=None, subtotals=None):
        if userId or product or quantity or subtotals:
            self.userId = userId
    ...
    def save(self):
        ...
        self.cursor.execute(
            "INSERT INTO sales(userId, title, quantity, subtotals, date) VALUES(%s,%s,%s,%s,%s)",
            (self.userId, self.title, self.quantity, self.subtotals, self.date),)
        self.conn.commit()
        self.conn.close()
